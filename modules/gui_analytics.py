...
class AnalyticsScreen:
    ...
    def create_widgets(self):
        ...
        title = tk.Label(
            header_frame,
            text=f"Performance Analytics - {self.username}",
            font=('Arial', 20, 'bold'),
            bg='#3498db',
            fg='white'
        )
        ...
        if user_history.empty:
            ...
            # Statistics panel
            self.create_stats_panel(scrollable_frame, user_history)
            
            # Graphs
            self.create_performance_trend_graph(scrollable_frame, user_history)
            self.create_category_performance_graph(scrollable_frame, user_history)
            self.create_difficulty_accuracy_graph(scrollable_frame, user_history)
            self.create_correct_incorrect_pie(scrollable_frame, user_history)
    ...
    def create_stats_panel(self, parent, df):
        ...
        for label, value in stat_items:
            stat_box = tk.Frame(stats_grid, bg='#ecf0f1', relief=tk.RAISED, bd=1)
            stat_box.grid(row=row, column=col, padx=10, pady=5, sticky='ew')
            
            tk.Label(stat_box, text=label, font=('Arial', 10), bg='#ecf0f1').pack()
            tk.Label(stat_box, text=str(value), font=('Arial', 12, 'bold'), bg='#ecf0f1').pack()
